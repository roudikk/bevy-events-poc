...
pub(crate) trait QueryHelper<'w, 's> {
    type QueryData: bevy_ecs::query::QueryData;

    fn get_some(
        &self,
        entity: Entity,
    ) -> Result<ROQueryItem<'_, 's, Self::QueryData>, QueryEntityError>;

    fn get_some_mut(
        &mut self,
        entity: Entity,
    ) -> Result<QueryItem<'_, 's, Self::QueryData>, QueryEntityError>;
}

impl<'w, 's, D: QueryData, F: QueryFilter> QueryHelper<'w, 's> for Query<'w, 's, D, F> {
    type QueryData = D;
