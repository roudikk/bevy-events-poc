...
impl WindowToEguiContextMap {
    ...
    pub fn on_egui_context_added_system(
        mut res: ResMut<Self>,
        added_contexts: Query<(Entity, &Camera), Added<EguiContext>>,
...
pub struct EguiContextsEventIterator<'a, E: bevy_ecs::event::BufferedEvent, F> {
...
impl<'a, E: bevy_ecs::event::BufferedEvent, F: FnMut(&'a E) -> Entity> Iterator
...
pub struct EguiContextEventReader<'w, 's, E: bevy_ecs::event::BufferedEvent> {
...
impl<'w, 's, E: bevy_ecs::event::BufferedEvent> EguiContextEventReader<'w, 's, E> {
...
pub fn write_window_pointer_moved_events_system(
    ...
    for (event, context) in cursor_moved_reader.read(|event| event.window) {
        let Ok((context_settings, mut context_pointer_position)) =
...
pub fn write_pointer_button_events_system(
    ...
    {
        let Ok((context_settings, context_pointer_position)) = egui_contexts.get_some(context)
        else {
            continue;
        };
...
pub fn write_mouse_wheel_events_system(
    ...
    for (event, context) in mouse_wheel_reader.read_with_non_window_hovered(|event| event.window) {
        ...
        let Ok(context_settings) = egui_contexts.get_some(context) else {
            continue;
        };
...
pub fn write_ime_events_system(
    ...
    for (event, context) in ime_reader.read_with_non_window_focused(|event| match &event {
        ...
        let Ok((_entity, context_settings, mut ime_state, _egui_output)) =
            egui_contexts.get_some_mut(context)
        else {
            continue;
        };
        ...
        egui_input.focused = focused_non_window_egui_context.as_deref().map_or_else(
            || {
                window_to_egui_context_map
                    .context_to_window
                    .get(&entity)
                    .and_then(|window_entity| windows.get_some(*window_entity).ok())
