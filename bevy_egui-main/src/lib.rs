...
use bevy::{prelude::DebugName, ui_render::graph::NodeUi::UiPass};
...
impl EguiContexts<'_, '_> {
    ...
    pub fn ctx_mut(&mut self) -> Result<&mut egui::Context, QuerySingleError> {
        self.q.iter_mut().fold(
            Err(QuerySingleError::NoEntities(DebugName::from(
                core::any::type_name::<EguiContextsPrimaryQuery>(),
            ))),
            |result, (ctx, primary)| match (&result, primary) {
                ...
                (Ok(_), Some(_)) => Err(QuerySingleError::MultipleEntities(DebugName::from(
                    core::any::type_name::<EguiContextsPrimaryQuery>(),
                ))),
                (Ok(_), None) => result,
            },
    ...
    #[cfg(feature = "render")]
    pub fn add_image(&mut self, image: AssetId<Image>) -> egui::TextureId {
    ...
    #[cfg(feature = "render")]
    #[track_caller]
    pub fn remove_image(&mut self, image: &AssetId<Image>) -> Option<egui::TextureId> {
...
pub struct RenderComputedScaleFactor {
    /// Scale factor ([`EguiContextSettings::scale_factor`] multiplied by [`bevy::prelude::Camera::target_scaling_factor`]).
    pub scale_factor: f32,
}
            ...
            if bevy_ui_is_enabled {
                ...
                let (below, above) = match self.ui_render_order {
                    UiRenderOrder::EguiAboveBevyUi => {
                        (UiPass.intern(), render::graph::NodeEgui::EguiPass.intern())
                    }
                    UiRenderOrder::BevyUiAboveEgui => {
                        (render::graph::NodeEgui::EguiPass.intern(), UiPass.intern())
                    }
                };
                if let Some(graph_2d) =
                    graph.get_sub_graph_mut(bevy_core_pipeline::core_2d::graph::Core2d)
                {
                    ...
                    match graph_2d.get_node_state(UiPass) {
                        Ok(_) => {
                ...
                if let Some(graph_3d) =
                    graph.get_sub_graph_mut(bevy_core_pipeline::core_3d::graph::Core3d)
                {
                    match graph_3d.get_node_state(UiPass) {
                        Ok(_) => {
...
pub fn setup_primary_egui_context_system(
    mut commands: Commands,
    new_cameras: Query<(Entity, Option<&EguiContext>), Added<bevy::prelude::Camera>>,
...
pub struct UpdateUiSizeAndScaleQuery {
    ...
    camera: &'static bevy::prelude::Camera,
}
